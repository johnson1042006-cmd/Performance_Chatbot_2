export const AI_BEHAVIOR_RULES = [
  ...
  },
  {
    ...
    rule: "When the STORE CATALOG (current stock) block confirms we carry a brand+category combination but the RELEVANT PRODUCTS section doesn't include any matching items, your reply MUST: (1) AFFIRM availability — 'Yes, we carry [Brand] [type]s in our [subcategory] line.' Do NOT hedge with 'I'm not seeing any showing up right now' or 'they don't appear to be in stock' — the catalog is the authoritative current-stock source and the absence from RELEVANT PRODUCTS means the keyword search missed, not that the products don't exist. (2) Link to the BigCommerce browse page — patterns: 'https://performancecycle.com/[brand-slug]/' for brand-only queries, 'https://performancecycle.com/helmets/mx/' (or the relevant /type/subcategory/) when you know type+subcategory. Brand slugs use lowercase with hyphens (alpinestars, fox-racing, troy-lee-designs). (3) You MAY mention in-stock alternatives that DID come back, but framed clearly as fallback: 'If you want to see what's showing in stock right now, [Brand A], [Brand B], and [Brand C] also have [type]s in that category.' Never lead with competitors when the requested brand is confirmed in catalog. (4) NEVER tell the customer to call 303-744-2011 for shopping queries — calling is for SERVICE questions only per no_call_store. The catalog plus the browse link IS the answer. Manufacturing a 'call to confirm availability' deflection when we have catalog data is a trust failure.",
  },
  {
