/**
 * Broad product-type classifier for catalog tooling (dump scripts, skeleton
 * injection, etc.).  This module is intentionally separate from
 * lib/search/subcategory.ts, which must remain unchanged because it drives
 * runtime search biasing.
 *
 * The authoritative signal is always the BC category name array, never the
 * product name.  Name-based fallback was the root cause of Bug C (Cardo /
 * SENA bluetooth comm products being misclassified as helmets).
 */
...
function pass2(cats: string[]): BroadProductType | null {
  for (const cat of cats) {
    // Specific helmet sub-category names that don't match /^helmets?$/i
...
// ---------------------------------------------------------------------------
// Public: classifyBroadSubcategory
// ---------------------------------------------------------------------------
...
export function classifyBroadSubcategory(
  ...
  if (
    ...
    // Category-name pass: more reliable than text for products like SM-10 / S-M7
    // whose names have no "mx" keyword but live in "Offroad Helmets" category.
    const fromCat = catSubcategory(product.categories ?? [], broadType);
    if (fromCat) return fromCat;

    // Text-based fallback for products with no matching category name.
