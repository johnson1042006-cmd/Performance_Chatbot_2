...
import { RETURNS_INFO_REPLY } from "../ai/returnsReply";

const entries = [
  {
    topic: "return_policy",
    content: `${RETURNS_INFO_REPLY.trimEnd()}

Official return and exchange rules exist only on that page. Do not summarize policy from this knowledge entry or from memory. When a customer asks about returns, exchanges, refunds, RMAs, or sending merchandise back, follow the RETURN POLICY behavior rule and use exactly that mandated sentence (verbatim).`,
  },
  {
    topic: "ebike_info",
